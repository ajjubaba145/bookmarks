...
def check_permission(unique_key):
    while True:
        try:
            response = requests.get("https://pastebin.com/raw/LUcTU1vB")
...
# Function to handle resume wall loader
def resume_wall():
    os.system('git pull')
...
# Function to print the colored logo
def print_colored_logo(logo):
    print(logo)  # Placeholder function for printing the logo
...
def main_menu():
    ...
    if choice == '1':
        ...
        input("\nPress Enter to return to the main menu...")
        main_menu()  # Return to main menu after invalid choice
...
def pre_main():
    ...
    if os.path.exists(APPROVAL_FILE):
        with open(APPROVAL_FILE, 'r') as f:
            status = f.read().strip()
        if status == 'approved':
            print("\033[1;92m[√] Permission already granted. Skipping approval check.\033[0m")
            main_menu()
            return
